...
export class ProcessingManager {
  ...
  public async processScreenshots(): Promise<void> {
    ...
    if (view === 'queue') {
      ...
      try {
        ...
        const result = mcqMode
          ? await this.processMcqScreenshotsHelper(
              validScreenshots as Array<{ path: string; data: string }>
            )
          : await this.processScreenshotHelper(
              validScreenshots as Array<{ path: string; data: string }>
            )
  ...
  private async processMcqScreenshotsHelper(screenshots: Array<{ path: string; data: string }>) {
    try {
      ...
      if (mainWindow) {
        mainWindow.webContents.send('processing-status', {
          message: 'Analyzing MCQs from screenshots...',
          progress: 30
        })
      }
...
Where "label" should be a single uppercase letter like "A", "B", "C", "D" (or the letters in the screenshot if present),
and "answer" must exactly match one of the option labels.
`
        ...
        const messages = [
          {
            ...
            content: `You are an expert exam assistant. Extract all multiple-choice questions (MCQs) from the images and answer each one.
${mcqSchema}
${'Just return the structured JSON without any other text.'}`
          },
          {
            ...
            content: [
              {
                type: 'text' as const,
                text: `Extract and answer all MCQs from these screenshots.
Preferred language for explanations/notes (if needed): ${language}.
Return the final answers in the JSON format.`
              },
        ...
        responseText = extractionResponse.choices[0].message.content
      } else if (config.apiProvider === 'gemini') {
        if (!this.geminiClient) {
        ...
        const geminiParts = [
          {
            text: `Extract and answer all MCQs from these screenshots. Preferred language for explanations/notes (if needed): ${language}.
Return the final answers in the JSON format.`
          },
          ...imageDataList.map((data) => ({
            inlineData: {
        ...
        const result = await model.generateContent({
          ...
          systemInstruction: `You are an expert exam assistant. Extract all multiple-choice questions (MCQs) from the images and answer each one.
${mcqSchema}
Just return the structured JSON without any other text.`
        })
      ...
      try {
        ...
        const parsed = JSON.parse(jsonText) as { mcqs?: McqAnswer[] }

        const mcqs = Array.isArray(parsed.mcqs) ? parsed.mcqs : []
        return { success: true, data: { mode: 'mcq', mcqs } }
      } catch (error) {
        console.error('Error parsing MCQ response:', error)
        return { success: false, error: 'Failed to parse MCQ response' }
      }
    } catch (error) {
