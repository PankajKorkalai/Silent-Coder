...
function App(): React.JSX.Element {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        if (typeof config?.mcqMode === 'boolean') {
          setMcqMode(config.mcqMode)
        }
  ...
  const handleMcqModeChange = useCallback(async (next: boolean) => {
    try {
      const result = await window.electronAPI.updateConfig({ mcqMode: next })
      if (result) {
        setMcqMode(next)
      }
    } catch (error) {
      console.error('Error updating mcqMode:', error)
      showToast('Error', 'Failed to update MCQ mode', 'error')
    }
  }, [showToast])
            ...
            {isInitialized ? (
              hasApiKey ? (
                <MainApp
                  ...
                  mcqMode={mcqMode}
                  setMcqMode={handleMcqModeChange}
                />
              ) : (
