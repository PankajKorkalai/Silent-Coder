...
interface QueueCommandsProps {
  ...
  mcqMode: boolean
  setMcqMode: (next: boolean) => void | Promise<void>
}
      ...
      <div className="pt-2 w-fit">
        <div className="text-xs text-white/90 backdrop-blur-md bg-black/10 rounded-lg py-2 px-4 flex items-center justify-center gap-4">
          <div
            ...
            title="When enabled, Solve will answer MCQs from your screenshots."
          >
            <div
              className={`w-9 h-5 rounded-full p-1 flex items-center transition-colors ${
                mcqMode ? 'bg-blue-500/70' : 'bg-white/10'
              }`}
            >
              <div
                className={`w-4 h-4 rounded-full bg-white transition-transform ${
                  mcqMode ? 'translate-x-4' : 'translate-x-0'
                }`}
              />
            </div>
